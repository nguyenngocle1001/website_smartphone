...
<table class="table table-bordered table-hover mt-3">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <div class="image" style="background-image: url({{URL::asset('storage/avatars/'.$users[$i]->UserId.'/'.$users[$i]->Avatar)}})"></div>
            </td>
            <td>{{ $users[$i]->Email }}</td>
            <td>{{ $users[$i]->Phone }}</td>
            <td>
                ...
                <form class="detroy__form" action="{{ route('adminroles.destroy', $users[$i]->UserId) }}" method="post">
                    @csrf
